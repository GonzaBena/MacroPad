...
exports.openThemesFolder = openThemesFolder;
exports.importExternalTheme = importExternalTheme;
...
function openThemesFolder() {
    ensureUserThemesDir();
    electron_1.shell.openPath(USER_THEMES_DIR);
}
// ── Theme Converter ──
function hexNormalize(color) {
    ...
    if (/^#[0-9A-Fa-f]{8}$/.test(s))
        return s.slice(0, 7); // strip alpha
    // 0x prefix
...
function fillMissingVars(colors, type) {
    ...
    const set = (key, value) => { if (!colors[key] && value)
        colors[key] = value; };
...
function convertVscodeTheme(data) {
    ...
    const colors = {};
    const v = (k, ...tokens) => { const h = pick(...tokens); if (h)
        colors[k] = h; };
