...
function hexNormalize(color: any) {
 ...
 */
function parseMinimalYaml(content: string) {
  const result: any = {};
  ...
  for (const rawLine of lines) {
    ...
    const parent: any = stack[stack.length - 1].obj;
 ...
 */
function parseMinimalToml(content: string) {
  const result: any = {};
  const lines = content.split(/\r?\n/);
  let currentPath: string[] = [];

  const getOrCreate = (obj: any, keys: string[]) => {
    let cur = obj;
    for (const k of keys) {
 ...
 */
function blendHex(hex: string, amount: number, isDark: boolean) {
  const h = hexNormalize(hex);
  if (!h) return null;
  const r = parseInt(h.slice(1, 3), 16);
  const g = parseInt(h.slice(3, 5), 16);
 ...
 */
function fillMissingVars(colors: any, type: string) {
  const isDark = type !== "light";
  const bg   = colors["--bg"];
  const text = colors["--text"];

  const set = (key: string, value: string | null) => { if (!colors[key] && value) colors[key] = value; };
  ...
  if (text && bg) {
    ...
    if (th && bh) {
      const mix = (t: number, b: number, f: number) => Math.round(t + (b - t) * f);
...
function pickHex(colorsObj: any, ...tokens: string[]) {
...
function buildColorEntry(result: any, varName: string, hex: string | null) {
  if (!hex) return;
  result[varName] = hex;
  result[`${varName}-bg`] = toRgba(hex, 0.12);
}

function convertVscodeTheme(data: any) {
  const c = data.colors || {};
  const pick = (...tokens: string[]) => pickHex(c, ...tokens);

  const colors: any = {};
  const v = (k: string, ...tokens: string[]) => { const h = pick(...tokens); if (h) colors[k] = h; };
...
function convertAlacrittyTheme(data: any) {
...
function convertFlatTheme(data: any) {
  // Windows Terminal / generic flat scheme: { background, foreground, red, green, ... }
  const pick = (...keys: string[]) => pickHex(data, ...keys);
  const colors: any = {};
...
function detectAndConvert(data: any) {
  let result;
...
export async function importExternalTheme(): Promise<any> {
  const win = getWindow();
